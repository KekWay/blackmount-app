...
interface ChatSettingsDeleteConfirmProps {
  show: boolean
  onCancel: () => void
  onConfirm: () => void
}

export function ChatSettingsDeleteConfirm({ show, onCancel, onConfirm }: ChatSettingsDeleteConfirmProps) {
  return (
    <AnimatePresence>
      {show && (
        <motion.div
        ...
        >
          ...
          >
            <div className="px-[24px] pt-[24px] pb-[16px] flex flex-col items-center gap-[12px]">
              <Image src="/assets/models/trash_icon.png" alt="" width={28} height={28} className="[filter:brightness(0)_saturate(100%)_invert(56%)_sepia(72%)_saturate(1054%)_hue-rotate(325deg)_brightness(101%)_contrast(94%)]" />
              <p className="text-[18px] text-white text-center font-bold">Удалить чат?</p>
              <p className="text-[13px] text-[rgba(255,255,255,0.45)] text-center leading-[20px] max-w-[280px]">
                Вся история диалога будет удалена безвозвратно
              </p>
            </div>
