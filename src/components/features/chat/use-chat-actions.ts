...
export function useChatActions(p: UseChatActionsParams) {
  ...
  const handleSend = () => {
    ...
    useBalanceStore.getState().addGenHistoryItem({ modelId: p.model.id, title: p.input.trim().slice(0, 100), preview: p.model.category === 'text' ? `Ответ от ${p.model.name}...` : '', time: timeStr, dateStr, type: p.model.category })
    const prefix = p.webSearchActive ? '\u{1F310} ' : ''
    p.setMessages((prev) => [...prev, { role: 'user', content: prefix + p.input }])
    p.setInput('')
    ...
    if (!p.isTextModel) {
      ...
      generationTimerRef.current = setTimeout(() => {
        p.setMessages((prev) => {
          ...
          if (li !== -1) {
            if (p.model.category === 'image' && p.imageCount > 1) { u[li] = { role: 'assistant', content: `Сгенерировано ${p.imageCount} изображений моделью ${p.model.name} (${p.selectedVersion.label})`, mediaType: 'image', mediaSrcs: testImagePool.slice(0, p.imageCount) } }
            else { u[li] = { role: 'assistant', content: `Сгенерировано моделью ${p.model.name} (${p.selectedVersion.label})`, mediaType: p.model.category as 'image' | 'video', mediaSrc: p.model.category === 'video' ? testVideoSrc : testImageSrc } }
          }
          return u
        })
      ...
      generationTimerRef.current = setTimeout(() => {
        p.setMessages((prev) => { const u = [...prev]; const li = u.findIndex((m) => m.isLoading); if (li !== -1) { u[li] = { role: 'assistant', content: txt, isTyping: true }; p.setTypingIdx(li) } return u })
      }, 2000)
    }
  }
  ...
  const handleNewChat = () => { p.setMessages([]); p.setGreeting(getRandomGreeting()) }
