...
export function ShareModal({ open, onClose, modelName }: ShareModalProps) {
  const [copied, setCopied] = useState(false)
  const shareUrl = `https://blackmount.ai/chat/shared/${Date.now()}`
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between px-[28px] pt-[24px] pb-[16px]">
          <p className="font-['Inter',sans-serif] font-extrabold text-[20px] text-white">Поделиться чатом</p>
        ...
        <div className="mx-[28px] mb-[24px] bg-[rgba(255,255,255,0.04)] rounded-[16px] p-[20px]">
          <div className="bg-[#131318] rounded-[12px] p-[16px] mb-[12px]">
            <p className="font-['Inter',sans-serif] font-normal text-[13px] text-[rgba(255,255,255,0.5)] leading-[20px]">
              Диалог с нейросетью {modelName} — Black Mount AI
            </p>
          </div>
          <p className="font-['Inter',sans-serif] font-extrabold text-[14px] text-white text-right">{modelName}</p>
        </div>
        <div className="flex items-center justify-center gap-[20px] px-[28px] pb-[28px]">
          <button onClick={handleCopy} className="flex flex-col items-center gap-[8px] group cursor-pointer">
            <div className={`size-[56px] rounded-full border flex items-center justify-center transition-all ${copied ? 'bg-[rgba(136,138,229,0.2)] border-[rgba(136,138,229,0.4)]' : 'bg-[rgba(255,255,255,0.06)] border-[rgba(255,255,255,0.1)] group-hover:bg-[rgba(255,255,255,0.1)]'}`}>
              {copied ? <Check size={20} className="text-[#888ae5]" /> : (
            ...
            <span className="font-['Inter',sans-serif] font-medium text-[11px] text-[rgba(255,255,255,0.5)]">{copied ? 'Скопировано!' : 'Копировать'}</span>
          </button>
          <a href={`https://t.me/share/url?url=${encodeURIComponent(shareUrl)}&text=${encodeURIComponent(`Чат с ${modelName} на Black Mount AI`)}`} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center gap-[8px] group cursor-pointer">
            ...
            <span className="font-['Inter',sans-serif] font-medium text-[11px] text-[rgba(255,255,255,0.5)]">Telegram</span>
          </a>
          <a href={`https://vk.com/share.php?url=${encodeURIComponent(shareUrl)}&title=${encodeURIComponent(`Чат с ${modelName}`)}`} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center gap-[8px] group cursor-pointer">
            <div className="size-[56px] rounded-full bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.1)] flex items-center justify-center group-hover:bg-[rgba(255,255,255,0.1)] transition-colors">
            ...
            <span className="font-['Inter',sans-serif] font-medium text-[11px] text-[rgba(255,255,255,0.5)]">VK</span>
          </a>
          <a href={`https://www.reddit.com/submit?url=${encodeURIComponent(shareUrl)}&title=${encodeURIComponent(`Чат с ${modelName} на Black Mount AI`)}`} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center gap-[8px] group cursor-pointer">
            <div className="size-[56px] rounded-full bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.1)] flex items-center justify-center group-hover:bg-[rgba(255,255,255,0.1)] transition-colors">
            ...
            <span className="font-['Inter',sans-serif] font-medium text-[11px] text-[rgba(255,255,255,0.5)]">Reddit</span>
          </a>
        </div>
