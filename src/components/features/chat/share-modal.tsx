...
export function ShareModal({ open, onClose, modelId, modelName, prompt, response, mediaType, mediaUrl }: ShareModalProps) {
  ...
  const shareTitle = `${modelName} — Black Mount AI`
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between px-[28px] pt-[24px] pb-[16px]">
          ...
          <button onClick={onClose} aria-label="Закрыть" className="group bg-[rgba(255,255,255,0.06)] rounded-[12px] size-[36px] flex items-center justify-center cursor-pointer hover:bg-[rgba(136,138,229,0.1)] transition-colors">
        ...
        <div className="mx-[28px] mb-[24px] bg-[rgba(255,255,255,0.04)] rounded-[16px] p-[20px]">
          <div className="bg-[#131318] rounded-[12px] p-[16px] mb-[12px]">
            <p className="font-manrope font-normal text-[13px] text-[rgba(255,255,255,0.5)] leading-[20px] line-clamp-2">
              {prompt || `Диалог с нейросетью ${modelName}`}
            </p>
          </div>
          <p className="font-manrope font-extrabold text-[14px] text-white text-right">{modelName}</p>
        </div>
        <div className="flex items-center justify-center gap-[20px] px-[28px] pb-[28px]">
          <button onClick={handleCopy} className="flex flex-col items-center gap-[8px] group cursor-pointer">
            ...
            <span className="font-manrope font-medium text-[11px] text-[rgba(255,255,255,0.5)]">{copied ? 'Скопировано!' : 'Копировать'}</span>
          </button>
          <a href={`https://t.me/share/url?url=${encodeURIComponent(shareUrl)}&text=${encodeURIComponent(shareTitle)}`} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center gap-[8px] group cursor-pointer">
          ...
          <a href={`https://vk.com/share.php?url=${encodeURIComponent(shareUrl)}&title=${encodeURIComponent(shareTitle)}`} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center gap-[8px] group cursor-pointer">
            <div className="size-[56px] rounded-full bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.1)] flex items-center justify-center group-hover:bg-[rgba(255,255,255,0.1)] transition-colors">
          ...
          <a href={`https://www.reddit.com/submit?url=${encodeURIComponent(shareUrl)}&title=${encodeURIComponent(shareTitle)}`} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center gap-[8px] group cursor-pointer">
            <div className="size-[56px] rounded-full bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.1)] flex items-center justify-center group-hover:bg-[rgba(255,255,255,0.1)] transition-colors">
