...
import { Copy, ThumbsUp, ThumbsDown, RotateCcw, Download } from 'lucide-react'
    ...
    <div className="flex flex-col">
      ...
      {msg.role === 'assistant' && !msg.isLoading && !msg.isTyping && (
        <div className="flex items-center gap-[4px] mt-[6px]">
          ...
          <button onClick={() => { const blob = new Blob([msg.content], { type: 'text/markdown' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'response.md'; a.click(); URL.revokeObjectURL(url) }} className="flex items-center gap-[4px] px-[8px] py-[4px] rounded-[8px] text-[11px] text-[rgba(255,255,255,0.3)] hover:text-white hover:bg-[rgba(255,255,255,0.06)] transition-all cursor-pointer"><Download size={11} /> Скачать</button>
